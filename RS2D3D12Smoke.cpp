...
static bool RS2D3D12_SmokeLifecycle(){
	...
	for(cycle = 0; cycle<RS2D3D12_SMOKE_CYCLES; cycle++){
		...
		// Twelve combinations: two position paths, with/without diffuse,
		// and no UV, UV-untextured, UV-textured. This proves every WP5
		// shader variant and the public layout descriptions at start-up.
		wsprintfA(label, "pipeline states cycle %d", cycle+1);
		RS2D3D12_SmokeStep(label, backend.GetPipelineStateCount()>=12);
