...
"	float2 ndc;\n"
"//	Direct3D 8 puts pixel centres on integer screen coordinates; Direct3D 10\n"
"//	and later put them half a pixel in.  The engine's 2D code is written for\n"
"//	the former (Fill2DRect subtracts 0.5 for it), so the half pixel is added\n"
"//	back here - without it every screen-space draw sat 0.5 px up and left\n"
"//	(found by the v0.1.5 shadow probe: the overlay missed its last row and\n"
"//	column).\n"
"	ndc.x = (input.pos.x + 0.5f)*g_Viewport.z*2.0f - 1.0f;\n"
"	ndc.y = 1.0f - (input.pos.y + 0.5f)*g_Viewport.w*2.0f;\n"
...
"		if(g_AlphaTest.z > 0.5f && g_AlphaTest.z < 1.5f)\n"
"		{\n"
"			if(alphaByte > g_AlphaTest.y) discard; // LESS_EQUAL\n"
"		}\n"
"		else if(g_AlphaTest.z > 1.5f)\n"
"		{\n"
"			if(alphaByte <= g_AlphaTest.y) discard; // GREATER\n"
"		}\n"
"	}\n"
"	return colour;\n"
"}\n";
